  ...
  <div class="login-container">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if (isset($_FILES['image']) && isset($_POST['caption'])) {
        ...
        $caption = (empty($_POST['caption'])) ?
          basename($file['name']) :
          trim(preg_replace('/[^a-zA-Z0-9]+/', '-', $_POST['caption']));
        // Ensure caption is not too long
        if (strlen($caption) > 56) {
          $caption = substr($caption, 0, 56);
        }
        // add timestamp to avoid filename collisions
        $caption .= '-' . date('Ymd-His');
        ...
        if (in_array($fileType, $allowedTypes)) {
          ...
          if (move_uploaded_file($file['tmp_name'], $targetFile)) {
            ...
            $galFilePath = $galleryDir . $newFileName;
            ...
            try {
              if ($fileType === 'heic') {
                ...
                imagejpeg($image, $newFilePath, $compSlide);
                imagejpeg($image, $galFilePath, $compGallery);
                imagedestroy($image);
              }
              // Move original file to processed folder
              rename($targetFile, $processedDir . $newFileName); // basename($file['name']));
